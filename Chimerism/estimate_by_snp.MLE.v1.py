...
if len(sys.argv) != 3:
	sys.exit('python3 %s <genotype_genomicDBI_gather.g.vcf.tab.snp.mat.filt> <seed>' % (sys.argv[0]))
...
np.random.seed(seed)
d1_array = np.random.choice(d1_array, 1000000)
np.random.seed(seed)
n_array = np.random.choice(n_array, 1000000)
